...
- **Borg, M. et al. (2026) — "Echoes of AI: Investigating the Downstream Effects of AI Assistants on Software Maintainability".** [arXiv](https://arxiv.org/abs/2507.00788). Two-phase controlled experiment (151 professional developers): Phase 1 added features with/without GitHub Copilot, Phase 2 had new developers evolve that code without AI. Found no significant differences in comprehensibility or maintainability. However, two important limitations: (1) studied Copilot (autocomplete), not agentic code generation — the human remains the author, so the comprehension gap from our row 2 does not apply; (2) underpowered for small-to-moderate effects (n=75 in Phase 2, power ~0.17 for small effects). Does not contradict our analysis — it studies a qualitatively different mode of AI use.
