...
This raises an interesting question about agents. LLMs are trained on an extremely broad corpus — code, documentation, postmortems across thousands of systems. In some sense, they possess *something* in the heuristic space too: statistical pattern recognition across far more systems than any individual human could experience. But there are qualitative differences. Human heuristics are weighted by lived consequences — the engineer who caused a production outage carries that experience with visceral weight that changes future behaviour. An LLM processes "payment flows are risky" the same way it processes "settings pages are low-risk" — as text, without asymmetric weighting from experience. Perhaps more critically, heuristics are most valuable when you know they *don't* apply — distinguishing "unusual because wrong" from "unusual because unique requirements demand it". An LLM knows what's statistically common but has a harder time recognising the valid exception. How LLM heuristics compare to human professional heuristics — in breadth, calibration, and exception recognition — is an open research question. We don't yet have a methodology for answering it.
...
**Business domain understanding is qualitatively degraded.** The agent may have broad statistical pattern recognition from training data — recognising common patterns across thousands of systems — but it lacks consequence-grounded calibration, contextual exception recognition, and project-specific accumulated knowledge. It has no knowledge of *this* business domain, *this* risk profile, *this* product lifecycle, or *this team's* consciously accepted tradeoffs. It can be given some context via prompts or configuration files, but it doesn't accumulate domain understanding over time the way a human does. A human developer who fixes a production incident in the payment flow carries that experience into every subsequent decision. An agent starts roughly fresh each session. Even when explicitly provided with domain context, the agent lacks the judgment that comes from having lived through the consequences of getting it wrong.
