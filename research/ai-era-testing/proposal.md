...
- **What is the minimum viable team composition in production?** The experiment fixes the full three roles (§5); production may not need them. With the product role defined by context and authority rather than title (§2), many teams already run as developer plus QA with only partial PM presence. Which degraded compositions still preserve the result, dev + QA, prepared PM input instead of a present PM, or a smaller team for lower-risk features, is a follow-up study, not something this proposal settles.
